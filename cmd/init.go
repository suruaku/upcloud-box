...
func buildCloudInit(user string, sshKeys []string) []byte {
	...
	b.WriteString("  - [sh, -c, \"sudo add-apt-repository ppa:cncf-buildpacks/pack-cli --yes\"]\n")
	b.WriteString("  - [sh, -c, \"sudo apt-get update\"]\n")
	b.WriteString("  - [sh, -c, \"sudo apt-get install pack-cli\"]\n")
